...
在编写递归时，我们需要知道如下几点：
- **递归节点的状态是什么。**
  - 我们常说的函数的状态，是指的函数被压入函数栈时，该函数栈中的一些数据信息，例如形参变量信息、该函数要用到的全局变量信息(例如路径path)等，这些就组成了一个状态
...
- **当前节点的下一个状态有哪些。**
  - 如果能获取一次性获取，则一次性获取，存储在`List<State> nextStateList`变量中。
    - 情况一：父状态如果要根据不同子状态所返回的内容做不同的处理，例如对于子状态1返回的内容要做A处理，对于子状态2返回的内容做B处理。则我们在获取状态时，可以给每种状态处理方式添加一个标识。例题：[最长公共子序列](https://leetcode.cn/problems/longest-common-subsequence/), [编辑距离](https://leetcode.cn/problems/edit-distance/)
...
单调递增栈：栈顶到栈底，元素单调递增。<font color="red">栈底元素是当前已访问元素里的最大的元素</font>
单调递减栈：栈顶到栈底，元素单调递减。<font color="red">栈底元素是当前已访问元素里的最小的元素</font>
...
例如现有数组`arr=[2,1,2,4,3]`，从arr[0]遍历至arr[3]，且将其放入单调递减栈中，效果如下：
...
```
 ...
 -------             
```
即可以看出1是arr[0]~arr[3]中元素最小的，2是arr[0]~arr[3]中第2个最小的，4是arr[0]~arr[3]中第3个最小的
...
class Solution {
  public int trap(int[] height) {
    int[] leftMaxIndex = new int[height.length], rightMaxIndex = new int[height.length];
    leftMaxIndex[0] = 0;
    rightMaxIndex[height.length - 1] = height.length - 1;
    for (int i = 1; i < height.length; i++) {
      leftMaxIndex[i] = height[leftMaxIndex[i - 1]] <= height[i] ? i : leftMaxIndex[i - 1];
    }
    for (int i = height.length - 2; i >= 0; i--) {
      rightMaxIndex[i] = height[rightMaxIndex[i + 1]] <= height[i] ? i : rightMaxIndex[i + 1];
    }
    int res = 0;
    for (int i = 1; i < height.length - 1; i++) {
      int minHight = Math.min(height[leftMaxIndex[i - 1]], height[rightMaxIndex[i + 1]]);
...
![](https://raw.githubusercontent.com/NaisWang/images/master/20230303192733.png)
...
class Solution {
  public String removeKdigits(String num, int k) {
    ...
    for (char numCh : numChs) {
      while (!stack.isEmpty() && k != 0 && stack.peek() > numCh) {
        stack.pop();
        k--;
      }
      stack.push(numCh);
...
public class MergeSort {
  private static void devide(int[] nums, int left, int right) {
  ...
  private static void merge(int[] nums, int left, int mid, int right) {
    ...
    while (i1 <= mid && i2 <= right) {
      nums[k++] = temp[i1] <= temp[i2] ? temp[i1++] : temp[i2++];
    }
    while (i1 <= mid) {
      nums[k++] = temp[i1++];
    }
...
```java
  /**
   * startState[0]: 表示节点编号
   * startState[1]: 表示源点到startState[1]节点的距离
   * 图的表示方式，这里是使用的带有的权值的邻接表的表示方式 
   */
  Integer[] dijkstra(Integer[] startState, List<Integer[]>[] graph) {
    ...
    while (!pq.isEmpty()) {
      Integer[] currentState = pq.poll();
      
      if (isSelected.contains(currentState[0])) continue;
      isSelected.add(currentState[0]);
      dist[currentState[0]] = currentState[1];
      
      List<Integer[]> nextStateList = new ArrayList();
      for (Integer[] neighbor : graph[currentState[0]]) {
        nextStateList.add(new Integer[]{neighbor[0], currentState[1] + neighbor[1]});
      }
      
      for (Integer[] nextState : nextStateList) {
        if (!isSelected.contains(nextState[0])) {
          pq.add(nextState);
        }
      }
...
class Solution {
  public int networkDelayTime(int[][] times, int n, int k) {
    // 邻接表存储图结构，同时存储权重信息。节点编号是从1开始的，所以要一个大小为n + 1的邻接表 
    List<Integer[]>[] graph = new List[n + 1];
    for (int i = 0; i <= n; i++) {
      graph[i] = new ArrayList();
    }
    for (int[] time : times) {
      graph[time[0]].add(new Integer[]{time[1], time[2]});
    }
    
    // 启动 dijkstra 算法计算以节点 k 为起点到其他节点的最短路径
    Integer[] dist = dijkstra(new Integer[]{k, 0}, graph);

    // 找到最长的那一条最短路径
    int res = 0;
    for (int i = 1; i <= n; i++) {
      if (dist[i] == null) return -1;
...
class Solution {
  ...
  public int dfs(int i, int j) {
    ...
    List<int[]> nextStateList = new ArrayList();
    if (text1Ch[i] == text2Ch[j]) {
      nextStateList.add(new int[]{i - 1, j - 1, 1});
    } else {
      nextStateList.add(new int[]{i - 1, j, 0});
      nextStateList.add(new int[]{i, j - 1, 0});
    }

    // 仔细仔细考虑nextState[2]的作用
    int res = 0;
    for (int[] nextState : nextStateList) {
      res = Math.max(res, nextState[2] + dfs(nextState[0], nextState[1]));
    }
    return dp[i][j] = res;
  }
}
...
```java
class Solution {
  char[] word1Chs, word2Chs;
  Integer[][] dp = new Integer[501][501];
  public int minDistance(String word1, String word2) {
    word1Chs = word1.toCharArray();
    word2Chs = word2.toCharArray();
    return dfs(word1Chs.length - 1, word2Chs.length - 1);
  }
  ...
  public int dfs(int i, int j) {
    ...
    if (dp[i][j] != null) return dp[i][j];
    List<int[]> nextStateList = new ArrayList();
    if (word1Chs[i] == word2Chs[j]) {
      nextStateList.add(new int[]{i - 1, j - 1, 0});
    } else {
      // 插入一个字符
      nextStateList.add(new int[]{i, j - 1, 1});
      // 删除一个字符
      nextStateList.add(new int[]{i - 1, j, 1});
      // 替换一个字符
      nextStateList.add(new int[]{i - 1, j - 1, 1});
    }
    ...
    int res = Integer.MAX_VALUE;
    for (int[] nextState : nextStateList) {
      res = Math.min(res, nextState[2] + dfs(nextState[0], nextState[1]));
    }
    return dp[i][j] = res;
  }
...
1 2 - 4 5 + *
```
与之对应的也有波兰表达式（也就是前缀表达式），和中缀表达式。

中缀表达式：操作符、运算符在需要计算的数值中间。这是我们习惯的计算表达式书写方式。
```
// 中缀表达式
(1-2)*(4+5)
```
前缀表达式（波兰表达式）：操作符、运算符在需要计算的数值前面。比如，`(1-2)*(4+5)`表示如下：
```
// 波兰表达式，即前缀表达式
* - 1 2 + 3 4
```
用抽象语法树的方式可以对上面三者有更直观的理解。还是`(1-2)*(4+5)`这个例子，其语法树如下：
```
     *
  /    \
 -      + 
...
1  2  3   4 
```
那么前缀表达式可以理解为前序遍历语法树，中缀表达式对应语法树的中序遍历，而逆波兰表达式也就是后序遍历语法树的结果。

对于人类而言，逆波兰表达式读起来很费劲。但是对于计算机而言，由于其操作顺序与计算机中栈（stack）数据结构非常适配，使得逆波兰表达式求值非常方便。也因此，我们本文使用逆波兰表达式实现具有加减乘除功能的计算器算法，思路会比直接用中缀表达式写计算器算法清晰很多。

### 逆波兰表达式求值
对逆波兰表达式观察可以发现，借助stack数据结构，对逆波兰表达式求值异常简洁。

计算方法：从左到右遍历整个表达式，如果遇到数字则直接入栈，如果遇到符号则将栈顶两个数字出栈。先出栈的在右边，后出栈的在左边，符号放中间，算出来的结果再压入栈顶。

模版题 150. 逆波兰表达式求值: https://leetcode.cn/problems/evaluate-reverse-polish-notation/description/

![](https://raw.githubusercontent.com/NaisWang/images/master/20230303222903.png)
  ...
  public int evalRPN(String[] tokens) {
    ...
    for (String token : tokens) {
      if (token.equals("-") || token.equals("+") || token.equals("*") || token.equals("/")) {
        ...
        if (token.equals("-")) numStack.push(temp2 - temp1);
        else if (token.equals("+")) numStack.push(temp2 + temp1);
        else if (token.equals("*")) numStack.push(temp2 * temp1);
        else if (token.equals("/")) numStack.push(temp2 / temp1);
      } else {
...
4. 遇到运算符时
   ...
   3. 如果：优先级比栈顶运算符 **低 或 相等**，将 s1 栈顶的运算符 **弹出**，并压入到 s2 中，再重复第 4.1 步骤，与新的栈顶运算符比较（因为 4.3 将 s1 栈顶运算符弹出了）这里重复的步骤在实现的时候有点难以理解，下面进行解说：
   1. 如果 **s1 栈顶符号** 优先级比 **当前符号** **高或则等于**，那么就将其 **弹出**，压入 s2 中（循环做，是只要 s1 不为空）如果栈顶符号为 `(`，优先级是 -1，就不会弹出，就跳出循环了
   2. 跳出循环后，则将当前符号压入 s1
5. 遇到括号时：
   1. 如果是左括号 `(` ：则直接压入 s1
...
下面进行举例说明：将中缀表达式：`1+((2+3)*4)-5` 转换为后缀表达式
...
我们其实可以把上述的运算符看成一个单调栈，栈底是优先级最低的

例题：224. 基本计算器: https://leetcode.cn/problems/basic-calculator/description/
...
class Solution {
  ...
  public int evel(List<String> RPE) {
    Stack<Integer> stack = new Stack();
    for (String str : RPE) {
      if (str.equals("-") || str.equals("+") || str.equals("*") || str.equals("/")) {
        int temp1 = stack.pop();
        int temp2 = stack.pop();
        if (str.equals("-")) stack.push(temp2 - temp1);
        else if (str.equals("+")) stack.push(temp2 + temp1);
        else if (str.equals("*")) stack.push(temp2 * temp1);
        else if (str.equals("/")) stack.push(temp2 / temp1);
      } else {
        int num = 0;
  ...
  public List<String> getRPE(String s) {
    List<String> res = new ArrayList();
    ...
    for (int i = 0; i < sChs.length; i++) {
      ...
      if (ch == '(') {
        ...
        while (operateStack.peek() != '(') {
          res.add(operateStack.pop() + "");
        }
        operateStack.pop();
      } else {
        while (!operateStack.isEmpty() && operateStack.peek() != '(' && getPriority(ch) <= getPriority(operateStack.peek())) {
          res.add(operateStack.pop() + "");
        }
        operateStack.push(ch);
      }
  ...
  private int[] parseInt(int i, char[] sChs) {
    int res = 0;
    while (i < sChs.length && sChs[i] >= '0' && sChs[i] <= '9') {
      res = res * 10 + (sChs[i] - '0');
      i++;
    }
...
![](https://raw.githubusercontent.com/NaisWang/images/master/20230224162718.png)
...
public List<List<Integer>> funExample(int[][] buildings) {
  ...
  events.sort((a, b) -> {
    if (a.pos == b.pos) {
      ...
      // 保证如果a与b为同一个id事件时，进入事件一定排在离开事件后面。
      if (a.id == b.id) return a.type == 1 ? -1 : 1;
...
注：排序时间时，如果两个事件的位置相同时，我们一定要先判断这2个是不是同一个事件，如果是的话，那么我们要保证进入事件一定排在离开事件后面。具体例子可以看[主持人调度（二）](https://www.nowcoder.com/practice/4edf6e6d01554870a12f218c94e8a299?tpId=295&tqId=1267319&ru=/exam/company&qru=/ta/format-top101/question-ranking&sourceUrl=%2Fexam%2Fcompany) 中，如果不加这句话的话，会过不了一些样例
```java
// 保证如果a与b为同一个id事件时，进入事件一定排在离开事件后面。
if (a.id == b.id) return a.type == 1 ? -1 : 1;
```
...
class Solution {
  public int[][] merge(int[][] intervals) {
    ...
    for (int i = 0; i < intervals.length; i++) {
      eventList.add(new int[]{i, intervals[i][0], 1});
      eventList.add(new int[]{i, intervals[i][1], -1});
    }

    eventList.sort((a, b) -> {
      if (a[1] == b[1]) {
        if (a[0] == b[0]) return a[2] == 1 ? -1 : 1; 
...
class Solution {
  public int[][] insert(int[][] intervals, int[] newInterval) {
    ...
    for (int i = 0; i < intervals.length; i++) {
      ...
      events.add(new int[]{i, interval[0], 1});
      events.add(new int[]{i, interval[1], -1});
    }
    events.add(new int[]{intervals.length, newInterval[0], 1});
    events.add(new int[]{intervals.length, newInterval[1], -1});
...
![](https://raw.githubusercontent.com/NaisWang/images/master/20230405204204.png)
...
public class Solution {
    ...
    public int minmumNumberOfHost (int n, int[][] startEnd) {
      ...
      for (int i = 0; i < startEnd.length; i++) {
        eventList.add(new int[]{i, startEnd[i][0], 1});
        eventList.add(new int[]{i, startEnd[i][1], -1});
      }
      
      eventList.sort((a, b) -> {
        if (a[1] == b[1]) {
          if (a[0] == b[0]) return a[0] == 1 ? -1 : 1;
          if (a[2] == 1 && b[2] == -1) return 1;
          if (a[2] == -1 && b[2] == 1) return -1;
        }
        return a[1] > b[1] ? 1 : -1;
