...
在编写递归时，我们需要知道如下几点：
- **递归节点的状态是什么。**
  - 如果状态很复杂，由多个变量组成，我们可以定义一个State类来表示状态, 
  - 如果节点状态是由若干个整型变量组成，且知道它们的取值范围时，则我们可以用**一维**数组来表示State，一维数组的长度就是节点状态的组成部分个数，state[0]表示节点状态第一个组成部分的取值，state[1]表示节点状态第二个组成部分的取值,依次类推下去
- **当前节点的下一个状态有哪些。**
  - 如果能获取一次性获取，则一次性获取，存储在`List<State> nextStateList`变量中。
  - 如果不能一次性，则分开获取。（ps：常见的不能一次性获取的情况：一个状态的下一个状态们之间不是独立的，例如第2个状态是什么状态与第1个状态遍历后息息相关，即如果你想要知道第2个状态是什么状态，则你必须要先遍历第1个状态，通过判断第1个状态遍历完后的结果来判断第2个状态是什么状态）
...
- **判断是否需要记录节点入栈时的一些信息。**如果需要，我们可以定义一个`pushStackInfo`全局变量
  ...
  - 如果节点状态使用数组表示的，则pushStackInfo也需要用数组表示
- **判断是否需要记录节点出栈时的一些信息。**如果需要，我们可以定义一个`popStackInfo`全局变量
  - 1. `Map<State, T> popStackInfo`或`Integer[]... popStackInfo`: 用来记录某节点**出栈时**的一些信息。例如：该节点的放回值
  - 如果节点状态使用数组表示的，则popStackInfo也需要用数组表示，数组的每个下标的取值代表节点状态的某个组成部分的取值
...
V dfs(State currentState) {
  ...
  for (State nextState : nextStateList) {
    if (!popStackInfo.containsKey(nextState)) {
      popStackInfo.put(nextState, dfs(nextState));
    }
...
V dfs(int[] currentState) {
  ...
  for (int[] nextState : nextStateList) {
    if (popStackInfo[nextState[0]]... == null) {
      popStackInfo[nextState[0]]... = dfs(nextState)
      ... // 拿到nextState节点返回信息作一些处理
    }
  }
...
class Solution {
  ...
  boolean dfs(Integer currentState) {
    if (arr[currentState] == 0) {
      return true;
    }
...
class Solution {
  ...
  public int findCircleNum(int[][] isConnected) {
    ...
    stateToComponentId = new Integer[isConnected.length]; 
  ...
  void dfs(int currentState, int currentComponentId) {
    for (int i = 0; i < isConnected[currentState].length; i++) {
      if (stateToComponentId[i] == null && isConnected[currentState][i] == 1) {
        stateToComponentId[i] = currentComponentId;
        dfs(i, currentComponentId);
      }
    }
  ...
  int dfs(Integer state) {
    ...
    for (Integer nextState : nextStateList) {
      if (!dp.containsKey(nextState)) {
        dp.put(nextState, dfs(nextState));
      }
      res = Math.max(res, dp.get(nextState) + 1);
    }
    return res;
  }
}
...
class Solution {
  ...
  int dfs(int i) {
    ...
    res = Math.max(res, res + dp.get(i - 1));
    return res;
  }
...
class Solution {
  ...
  int[] dfs(int i) {
    ...
    res[0] = Math.min(nums[i], Math.min(nums[i] * dp.get(i - 1)[0], nums[i] * dp.get(i - 1)[1]));
    res[1] = Math.max(nums[i], Math.max(nums[i] * dp.get(i - 1)[0], nums[i] * dp.get(i - 1)[1]));
    return res;
  }
...
class Solution {
  ...
  boolean dfs(int currentState) {
    ...
    List<Integer> nextStateList = new ArrayList();
    for (int j = currentState; j >= 0; j--) {
      if (wordDictSet.contains(s.substring(j, currentState + 1))) {
        nextStateList.add(j - 1);
      }
    }
...
class Solution {
  ...
  int dfs(int i) {
    ...
    List<Integer> nextStateList = Arrays.asList(i - 1, i - 2);
    int res = 0;
    for (Integer nextState : nextStateList) {
      if (!dp.containsKey(nextState)) {
        dp.put(nextState, dfs(nextState));
      }
      res += dp.get(nextState);
    }
    return res;
  }
}
...
class Solution {
  ...
  public int longestArithSeqLength(int[] nums) {
    ...
    int res = 0;
    for (int i = nums.length - 1; i >= 0; i--) {
      res = Math.max(res, dfs(new int[]{i, Integer.MAX_VALUE}));
    }
    return res;
  }
  
  Integer[][] popStackInfo = new Integer[1002][1002];
  int dfs(int[] state) {
    ...
    for (int i = state[0] - 1; i >= 0; i--) {
      if (state[1] == Integer.MAX_VALUE) {
        nextStateList.add(new int[]{i, nums[i] - nums[state[0]] + 501}); 
      } else if (nums[i] - nums[state[0]] + 501 == state[1]) {
        nextStateList.add(new int[]{i, state[1]});
      }
    }

    for (int[] nextState : nextStateList) {
      if (popStackInfo[nextState[0]][nextState[1]] == null) {
        popStackInfo[nextState[0]][nextState[1]] = dfs(nextState);
      }
      res = Math.max(res, 1 + popStackInfo[nextState[0]][nextState[1]]);
    }
    return res;
  }
...
void slidingWindow(int[] nums) {
  // left, right表示2个指针的指向 
  // 1. 例如在滑动窗口中表示为左闭右闭区间，即[left, right]
  // 2. 例如在2个链表中，left表示指向第一个链表的下标；right表示执行第二个链表的下标
  int left = ..., right = ...;
...
<font color="red">所有的双指针问题，我们都可以写成进入while循环的第一步就是将其中一个指针移动一下</font>
...
class Solution {
  public boolean checkInclusion(String s1, String s2) {
    char[] s1Ch = s1.toCharArray(), s2Ch = s2.toCharArray();
    int[] targetchCnt = new int[27], nowChCnt = new int[27];
    for (char ch : s1Ch) {
      targetchCnt[ch - 'a']++;
    }

    int left = 0, right = -1;
    while (right + 1 < s2Ch.length) {
      right++;
      int newAddCh = s2Ch[right] - 'a';
      nowChCnt[newAddCh]++;
      while (left <= right && nowChCnt[newAddCh] > targetchCnt[newAddCh]) {
        nowChCnt[s2Ch[left] - 'a']--;
        left++;
      }
...
class Solution {
  public List<Integer> findAnagrams(String s, String p) {
    ...
    int[] targetCharCnt = new int[27], nowCharCnt = new int[27];
    for (char ch : pCh) {
      targetCharCnt[ch - 'a']++;
    }
    ...
    while (right + 1 < sCh.length) {
      right++;
      int newAddChar = sCh[right] - 'a';
      nowCharCnt[newAddChar]++;
      while (left <= right && nowCharCnt[newAddChar] > targetCharCnt[newAddChar]) {
        nowCharCnt[sCh[left] - 'a']--;
        left++;
      }
      if (Arrays.equals(targetCharCnt, nowCharCnt)) {
        res.add(left);
      }
    }
    return res;
  ...
  public String minWindow(String s, String t) {
    ...
    while (right + 1 < sCh.length) {
      ...
      if (validCnt == tMap.keySet().size()) {
        if ((res[0] == null && res[1] == null) || res[1] - res[0] > right - left) {
          res[0] = left;
          res[1] = right;
        }
      }
    }
    return res[0] == null ? "" :  s.substring(res[0], res[1] + 1);
  }
}
```
...
class Solution {
  public int longestSubstring(String s, int k) {
    ...
    for (int i = 1; i < 27; i++) {
      ...
      while (right + 1 < sCh.length) {
        right++;
        map.put(sCh[right], map.getOrDefault(sCh[right], 0) + 1);
        if (map.get(sCh[right]) == k) validCnt++;
...
class Solution {
  ...
  public int characterReplacement(String s, int k) {
    ...
    while (right + 1 < sCh.length) {
      right++;
      chCnt[sCh[right] - 'A']++;
      while (left <= right && right - left - getMaxCnt(chCnt) + 1 > k) {
        chCnt[sCh[left] - 'A']--;
        left++;
      }
      res = Math.max(res, right - left + 1);
  ...
  public int getMaxCnt(int[] chCnt) {
    return Arrays.stream(chCnt).max().getAsInt();
  }
