...
export var hljs = function () {
      ...
      const E = "[a-zA-Z]\\w*", y = "[a-zA-Z_]\\w*", N = "\\b\\d+(\\.\\d+)?",
          w = "(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)", v = "\\b(0b[01]+)", O = {
      ...
      var ne = (e => {
        ...
        function f(e, t, i, r) {
          ...
          function c() {
            ...
            let n = x.keywordPatternRe.exec(S), t = ""
            ;
            for (; n;) {
              t += S.substring(e, n.index)
              ;const i = N.case_insensitive ? n[0].toLowerCase() : n[0], r = (a = i, x.keywords[a]);
              if (r) {
                const [e, a] = r
                ;
          ...
          function _(e) {
            const n = e[0], a = t.substr(e.index), i = m(x, e, a);
            if (!i) return ee;
            const r = x
      ...
      const we = e => m(/\b/, e, /\w$/.test(e) ? /\b/ : /\B/), ve = ["Protocol", "Type"].map(we),
          Oe = ["init", "self"].map(we), xe = ["Any", "Self"],
